...
int main (int argc, char** argv) {
  ...
  hsize_t NX = N[0], NY = N[1]; // if N = 10000
  hsize_t Nlocal[2] = {NX/dim[0], NY/dim[1]}; // {2500, 2500}
  hsize_t Nlocal1[2] = {NX/(dim[0]/2), NY/(dim[1]/2)}; //{5000, 5000}
  hsize_t offset[2] = {mpirank / (dim[0]/2), mpirank % (dim[0]/2)};
  for(int i=0; i<2; i++) offset[i] *= Nlocal[i];
  hsize_t count[2] = {2,2};
  hsize_t stride[2] = {Nlocal1[0],Nlocal1[0]}; //{5000, 5000}
  ...
  H5Pclose(plist);
  vector<int> buffer(Nlocal1[0]*Nlocal1[1]);
  ...
  int sum = 0;
  for (int i=0; i<Nlocal1[0]*Nlocal1[1]; i++)
    sum += buffer[i];
  printf("sum=%d\n",sum);
